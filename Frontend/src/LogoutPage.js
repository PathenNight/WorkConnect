...
const LogoutPage = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            clearInterval(interval);  // Clear the interval when the component is unmounted or redirected
            clearTimeout(timer);  // Clear the timeout to prevent any potential issues if user leaves early
        };
    }, [navigate]);
